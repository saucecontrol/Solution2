...
#if defined(_WIN32)
#include <windows.h>
#define libPath "csharp-go-brrr.dll"
#define libLoad(lib) LoadLibraryA(lib);
#define symLoad GetProcAddress
#define errLoad GetLastError
#pragma comment (lib, "kernel32.lib")
#else
#include <dlfcn.h>
#define libPath "csharp-go-brrr.so"
#define libLoad(lib) dlopen(lib, RTLD_LAZY);
#define symLoad dlsym
#define errLoad dlerror
#endif

typedef void(*convFunc)(std::uint32_t*, std::size_t);
...
namespace v6 {
    ...
    inline void premultiply_alpha_simd(uint32_t* data, size_t pixel) noexcept {
#if !defined(NSIMD) && defined(__clang__)
#    if defined(__i386__) || defined(__x86_64__)
